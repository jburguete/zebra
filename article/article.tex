...
\usepackage[textsize=scriptsize,textwidth=3.5cm]{todonotes}
...
networks. Larvae settle in pipeline walls creating relevant obstructions to flow
as they grow and develop shells. Oxidant injections in the stream are commonly
used to control the infestation. A model is proposed for simulating the
transportation, settlement and death of mussels in pressurized networks. This
model is coupled to a solute transport, diffusion, and decay model of oxidant
chemicals. During the analyzed irrigation campaigns, the larvae entry was
monitored at various intervals. These data, showing high variability, were used
as input to the model using different scenarios averaging and modifying the time
distribution of larvae concentration. Simulations predicted similar mussel
...
the effectiveness of continuous and intermittent oxidant applications.
Continuous treatments were the most effective (up to 99\%), but required up to
3.5~kg$\cdot$ha$^{-1}$ of chlorine. Reasonable control could also be attained
with short injections (1 to 3 hours) just before the peak irrigation service
discharge, leading to up to 93\% of chlorine savings and reaching similar
mortality rates. The model was also used to estimate the larvae and chlorine
export to on-farm irrigation systems through hydrants and to evaluate strategies
for mitigating the risks of on-farm infestation and environmental impact. The
protection of on-farm irrigation systems required additional chlorine input. The
model can be parametrized to simulate similar species in different types of
pressurized networks, using different chemicals for treatment.
\end{abstract}

\begin{highlights}
\item
We introduced a model for zebra mussel dispersion, settlement and death by
oxidants
\item
The distribution of settled mussels depends more on hydraulics than on larval
input
\item
Short treatments are more effective when applied before the peak daily discharge
\item
On-farm protection needs more chlorine concentration than at the collective
network
\end{highlights}

\begin{keyword}
invasive, species, larva, veliger, shell, oxidant, pipeline
\end{keyword}

\end{frontmatter}
\todo{Comment R1-1: The keywords have been revised to avoid using terms already
present in the title}
\todo{Comment R3-1: The abstract has been shortened to less than 250 words}
...
$Ad$ & = & advection-diffusion substep (-),\\
...
Four larval stages have been defined for zebra mussels: embryonic, trochophore,
veliger and pediveliger stage \citep{Cohen09}. The veliger phase can last for
one or more weeks, while the other three phases may last for a few hours. The
warmer and more productive the water, the shorter the larval period. These
stages are planktonic, meaning that larvae can drift in water currents,
potentially reaching long distances. However, larval capacity to swim in water
is very limited, and can be considered negligible. Pediveligers settle by
attaching themselves to a substrate with byssal threads, becoming juvenile
mussels. After a few months, mussels evolve to sexually mature adults. Females
can lay between forty thousand and a million eggs annually \citep{CHE07}. In the
conditions of northern Spain, juveniles appear at the beginning of summer,
reaching high concentrations in water. A second peak has been observed at the
end of summer \citep{Araujo06, URA14}. Adults can reach up to 1.5~cm in their
first year of life, and a maximum size of 3~cm.
...
chemical treatment of zebra mussel infestation. 
The method also detects the
presence of dead shells freely moving with water inside the pipes, 
collapsing filters dozens
of times a day. 
...
Chlorination levels have also been assessed for
...
\end{enumerate}
\todo{Comment R4-1: the introduction has been revised to be more concise}
...
\citet{MackieClaudi10} stated that chlorine concentrations between 1 and
$3\cdot10^{-3}$~kg$\cdot$m$^{-3}$ are effective at immobilizing or killing
incoming veligers.  \citet{SprecherGetsinger00} reported that the relationship
between concentration and exposure time was typically inversely linear,
achieving 100\% larval mortality with a chlorine concentration of
$0.5\cdot10^{-3}$~kg$\cdot$m$^{-3}$ in 2 hours.However, for adults,
a concentration of $2.0\cdot10^{-3}$~kg$\cdot$m$^{-3}$ was required.
...
case:
\EQ{\exp\PA{-7200\,\tau}=0.01\Rightarrow\;\tau=6.4\cdot10^{-4}\;\textrm{s}^{-1}}
{EqMusselDecayTau}
...
are divided into advection-diffusion~($Ad$), solute decay~($D$),
larvae settlement~($Se$), mussel mortality~($M$), junctions~($J$), and
inputs~($I$) substeps:
\todo{Comment R1-2: $Ad$ is now used for advection-diffusion and $Se$ for larvae
settlement to avoid confusion}
\EQ{\Delta\vec{u}_i^n=\Delta\vec{u}_i^{Ad}+\Delta\vec{u}_i^D
	+\Delta\vec{u}_i^{Se}+\Delta\vec{u}_i^M+\Delta\vec{u}_i^J
	+\Delta\vec{u}_i^I}{EqSplit}
where $\vec{u}_i^{Ad}=\vec{u}_i^n+\Delta\vec{u}_i^{Ad}$,
$\vec{u}_i^D=\vec{u}_i^{Ad}+\Delta\vec{u}_i^D$,
$\vec{u}_i^{Se}=\vec{u}_i^D+\Delta\vec{u}_i^{Se}$,
$\vec{u}_i^M=\vec{u}_i^{Se}+\Delta\vec{u}_i^M$,
...
The EPANET software \citep{EPANET}
\todo{Comment R1-3: The EPANET reference is now the github page of the project}
was used to determine the pressure at the
network nodes and the discharge at the network pipes to satisfy hydrant
...
\[-\theta\,K_x\,\frac{\Delta t}{\delta x}\,\Delta\vec{u}_{i-1}^{Ad}
...
tridiagonal system of equations in $\Delta\vec{u}_i^{Ad}$ that is solved by
...
the second a condition on the implicit parameter.

\todo{This is now better explained in subsection 2.2.9}

\subsubsection{Model of solute decay}

The solute decay equation (\ref{EqSoluteDecay}) can be solved analytically
within a time step, resulting in:
\EQ{s_i^J=s_i^{Ad}\,\exp\CO{-\PA{K_b-K_w\,\frac{P}{A}}\,\Delta t}}{EqStepDecay}
...
\EQ{p_i^{Se}=-A\,\delta x\,\Delta c_i^{Se}=V\,P\,c_i^J\,\Delta t\,\delta x}
{EqMusselSettlementProbability}
We generate a pseudo-random number $R\in[0,1)$ and if $R<p_i^{Se}$, we add an
...
\EQ{c_i^{Se}=\max\PA{0,\;c_i^J-\frac1{A\,\delta x}}}
{EqMusselSettlementConcentration}
Additionally, if $N_i^{Se}$ is the number of mussels in the cell list, the
surface density of mussels will be:
\EQ{\sigma_i^{Se}=\frac{N_i^{Se}}{P\,\delta x}}{EqMusselSettlementDensity}
This stochastic method only makes sense if $p_i^{Se}<1$. This leads to the
...
\EQ{c_i^M=c_i^{Se}\,\exp\PA{-\frac{\tau\,s\,\Delta t}{s_m}}}{EqStepDeath}
...
order:
\EQ{d_i^M=\frac{\Delta\sigma_i^M}{\sigma_i^{Se}}
	=\frac{\tau\,s_i^{Se}\,\Delta t}{s_m}
...
\EQ{\Delta t<\frac{s_m}{\tau\,s_i^{Se}}}{EqMusselDeathTimeStep}
...
\subsubsection{Initial conditions \label{SecInitialConditions}}
...
Finally, from restrictions (\ref{EqCFL}), (\ref{EqMusselSettlementTimeStep}) and
(\ref{EqMusselDeathTimeStep}), to balance accuracy and computational efficiency
while preserving TVD characteristics, the following parameter values were
employed:
\EQ{\Delta t^n=0.9\,\min_i\PA{\frac{\delta x}{|v|},\;
	\frac{1}{V\,P\,c\,\delta x},\;
	\frac{s_m}{\tau\,s}}_i^n,\quad
	\theta^n=\max_i\PA{\frac12,\;1-\frac{\delta x^2}{2\,K_x\,\Delta t}}_i^n}
{EqTheta}
...
(Huesca, Spain), was selected for this study (see location of the area in
Figure~\ref{FigMap}). This WUA makes part of the Riegos
...
\FIGB{0.7\textwidth}{Montesusin.png}{Map displaying the location of Montesusín
in Spain (source: Wikipedia)}{FigMap}
\todo{Comment R4-2: a map has been provided (Fig.~1)}
...
Chemical control programs can be implemented continuously, periodically, or as
end-of-season treatments. In this study, continuous and periodic chlorine-based
control programs were assessed. These proactive approaches are designed to
prevent zebra mussel attachment within the pipeline network. A dosing pump was
used to inject the chemical, calibrated according to the flow rate of the
network and the concentration of the chemical solution, to maintain a consistent
chlorine level of $2\cdot10^{-3}$~kg$\cdot$m$^{-3}$.
\todo{Comment R3-5: A brief information on application of chlorine has been
included}

The network is treated with high concentrations of oxidants every year at least
once, at the end of the irrigation season. Further, the elevated reservoir is
desiccated during winter to expose mussels to lethal dry and cold conditions.
Consequently, the initial condition of the model
(subsection~\ref{SecInitialConditions}) is valid for this case study.
\todo{Comment R4-3: this paragraph has been added to clarify the initial
conditions}

Larvae intake measurements into the network were conducted at an accessible and
representative point of the area most affected by zebra mussel colonization. An
automatic sampling device connected to the network via an electro-valve was
used. The sampling equipment consists of a programmer, a volumetric counter, and
an internal hydraulic circuit that allows the collection of up to 10 independent
samples. The programmer controls the opening and closing of the electro-valve
according to the sampling schedule set by the user. Each time a sample is taken,
100 liters of water are filtered throughone of the ten available filters. The
filters fill sequentially. Samples were collected once per week and, when a
sample is collected, a chemical fixative is applied to preserve it. During the
2021 campaign, sampling began on July 25 and ended on August 19, with samples
taken every 12 hours, specifically at 12:00 and 24:00 each day, except on
weekends. In 2022, sampling began on June 14 and ended on July 4, with samples
taken every 8 hours, specifically at 1:00, 9:00, and 17:00 each day. In 2023,
sampling extended from May 12 to June 9, with samples taken every 12 hours at
3:00 and 15:00 each day. The determination of larval concentration was
outsourced to a specialized laboratory of the OX-CTA company
(\url{https://grupoox.com}), which performed the analyses using the
standardize method of cross-polarized light microscopy \citep{Frischer12}.
\todo{Comments R3-3 \& R3-8: the methodology for monitoring the entry of zebra
mussel larvae into the network has been described in much greater detail}

In order to analyze the effect that the larval inputs could have on the
CS-WUA~\#1 network infestation, we simulated four larval input scenarios during
two irrigation campaigns, 2021 and 2022, keeping the total number of larvae in
each campaign as measured:
\begin{enumerate}[I.]
\item larval input as measured (the concentration at each hour was temporally
...
concentrations, and hourly time intervals throughout the entire campaign. For
random input scenarios, to achieve variations of three orders of magnitude while
maintaining the number of larvae, concentrations were generated using the
formula:
\EQ{c_i=\frac{k}{0.001+R_i},\quad
	k=\frac{N_l}{\sum_i\frac{Q_i\,\Delta t_i}{0.001+R_i}}}{EqLarvaeRandom}
with $k$ being a constant and $R_i\in[0,1)$ pseudo-random numbers. For constant
larval concentration scenarios:
\EQ{c_i=\frac{N_l}{\sum_iQ_i\,\Delta t_i}}{EqLarvaeConstant}
\todo{Comment R3-9: formulae 23 to 25 are moved from Results and discussion to
Material and methods section}

The effect of larval input scenarios and oxidant treatments in the network will
be quantified using the following coefficients with respect to the reference
case:
\begin{description}
	\item[$\Delta N_t$,] the increase in the total number of mussels
		settled to the pipe walls expressed as a percentage,
	\item[$r_1$,] the correlation coefficient comparing the surface density
		of settled mussels in every cell, and
	\item[$r_2$,] the correlation coefficient comparing the surface density
		of settled mussels averaged per pipeline.
\end{description}
\todo{Comment R3-4: coefficients of the effectiveness of the oxidant
application aproaches has been eplained in this section}
...
the network hydrants in the CS-WUA \#1 network.
...
ranging between 20 and 25$^\circ$C.
...
Next, we analyze the effect that the variable larval inputs could have on
the CS-WUA \#1 network infestation.
The variables used in each scenario are detailed in Table~\ref{TabLarvaeInput}.
Note the large number of estimated larvae entering the system
in each campaign, on the order of one billion. Due to the lack of data, a zero
larval input has been assumed for each irrigation campaign during periods
outside the measurement window. Given that the measurement intervals during each
campaign were brief, it is likely that the actual number of incoming larvae was
higher, especially considering that each campaign measured during different
times of the year yielded larval concentrations of the same order of magnitude.
The average larval concentration measured in 2022 was nearly four times higher
than that of 2021, as shown by the constants $c_i$ and $k$ for scenarios II and
III in Table 1, respectively. However, no definitive conclusions can be drawn
due to the significant variability in the data. Moreover, the total number of
larvae was only about double, primarily because the measurement window in 2022
was shorter and water demands during the 2021 measurement period were lower.
\todo{Comment R3-7: The paragraph explaining Table 1 has been expanded to
clarify the values of the constants presented in the table}
...
campaigns. A strong random character is observed in this variable, with sudden,
strong variations of up to three orders of magnitude.

\FIGVIII{Figure_3a.pdf}{Figure_3b.pdf}{Figure_3c.pdf}{Figure_3d.pdf}
{Figure_3e.pdf}{Figure_3f.pdf}{Figure_3g.pdf}{Figure_3h.pdf}
{Measured (I) and simulated (II-IV) larval density input at the network inlet
over time (years 2021 and 2022). Each subfigure corresponds to a different
scenario \label{FigMusselInput2022}}
...
comparison are presented respect to the scenario IV results ($\Delta N_t$, $r_1$
and $r_2$).
\todo{Comment R3-4: these coefficients are explained in the M\&M subsection 2.3}
Scenario IV exhibits the
...
In a third simulation analysis, we addressed the effect of the chlorine
...
detect relationships with temperature or date of measurement which could have
explained such variations. These results are consistent with other studies
showing strong variability in larval presence, with low correlations between
larvae density, adult population or temperature \citep{Alix16, AlonzoMoya21}.
This confirms the usefulness of the simulations for scenario III.
Mussels settlement seems to be primarly associated to the hydraulic
...
treatments can mitigate this issue. Despite the many sources of uncertainty in
the model application, we believe that it can support the control zebra mussel
and similar invasive species in specific networks. Our recommendation is to
consider simulation results more qualitatively than quantitatively. Intensive
use of experimental data would be required to push the model one step further,
calibrating and validating it in real conditions.
\todo{Comment R2-1: Future recommendation has been added at end of conclusion
remark}
\todo{Comments R2-3, R3-6 and R4-5: We did not set out to extract general
conclusions about the chlorine treatment, but to present a model and evaluate
its capacity to analyze population dynamics under oxidant treatments}
